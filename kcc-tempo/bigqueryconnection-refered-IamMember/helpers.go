...
// MemberFrom can only have oneOf a ServiceAccountRef, a LogSinkRef, a SQLInstanceRef, a BiqgueryConnectionRef,
// so to resolve these values, it is necessary to call on the TFIAMClient
func ResolveMemberIdentity(ctx context.Context, member v1beta1.Member,
	...
	if memberFrom.BiqgueryConnectionRef != nil {
		refs = append(refs, memberFrom.BiqgueryConnectionRef)
		gvks = append(gvks, BigqueryConnectionGVK)
	}
	...
	return id, fmt.Errorf("%v memberFrom refs found. Exactly one Of 'logSinkRef', 'serviceAccountRef', 'sqlInstanceRef', 'serviceIdentityRef', 'bigqueryConnectionRef' must be used", len(refs))
}
