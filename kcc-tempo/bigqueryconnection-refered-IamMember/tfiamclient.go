...
func resolveTargetFieldValue(r *krmtotf.Resource, targetField string) (string, error) {
	var key string
	// targetField may contain nested fields
	if r.GroupVersionKind().Kind == BigqueryConnectionKind {
		key = ""
		fields := []string{}
		for _, field := range strings.Split(targetField, ".") {
			fields = append(fields, text.SnakeCaseToLowerCamelCase(field))
		}
		key = "observedState." + strings.Join(fields, ".")
	} else {
